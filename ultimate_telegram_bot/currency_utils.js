...
async function getRatesFromResponse(ratesPromise) {
    ...
    await ratesPromise
        .then((r) => {
            const data = r.data;
            let usdRate = data.find(obj => {return obj["currencyCodeA"] === 840;});
            let eurRate = data.find(obj => {return obj["currencyCodeA"] === 978;});
            ...
            if (usdRate && eurRate) {
                ...
                usd = data.find(obj => {return obj["ccy"] === "USD"});
                usd.sell = usd.sale;
                delete usd.sale;
                eur = data.find(obj => {return obj["ccy"] === "EUR"});
                eur.sell = eur.sale;
                delete eur.sale;
            }
        })
